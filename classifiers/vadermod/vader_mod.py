...
def scalar_inc_dec(word, valence, is_cap_diff):
    ...
    if word_lower in BOOSTER_DICT:
        scalar = BOOSTER_DICT.get(word_lower)
        if valence < 0:
            scalar *= -1
...
class SentiText(object):
    ...
    def _words_and_emoticons(self):
        words_emos = self.text.split()

        # get rid of residual empty items or single letter words
        words_emos = [we for we in words_emos if len(we) > 1]

        for word in self._words_only():
            for punct in PUNC_LIST:
                pword = punct + word
                u1 = words_emos.count(pword)
                while u1 > 0:
                    i = words_emos.index(pword)
                    words_emos.remove(pword)
                    words_emos.insert(i, word)
                    u1 = words_emos.count(pword)

                wordp = word + punct
                u2 = words_emos.count(wordp)
                while u2 > 0:
                    i = words_emos.index(wordp)
                    words_emos.remove(wordp)
                    words_emos.insert(i, word)
                    u2 = words_emos.count(wordp)
        return words_emos
...
class SentimentIntensityAnalyzer(object):
    ...
    def __init__(self, lexicon_file="vader_lexicon.txt"):
        ...
        lexicon_full_filepath = join(
            dirname(_this_module_file_path_), lexicon_file)
        ...
        if item_lowercase in self.lexicon:
            ...
            for start_i in range(0, 3):
                if i > start_i and words_and_emoticons[i - (start_i + 1)].lower() not in self.lexicon:
                    ...
                    s = scalar_inc_dec(words_and_emoticons[
                                       i - (start_i + 1)], valence, is_cap_diff)
    ...
    def _idioms_check(self, valence, words_and_emoticons, i):
        ...
        if len(words_and_emoticons) - 1 > i + 1:
            zeroonetwo = "{0} {1} {2}".format(words_and_emoticons[i], words_and_emoticons[
                                              i + 1], words_and_emoticons[i + 2])
    ...
    def score_valence(self, sentiments, text):
        """ Get dict of sentiment scores, compound, pos, neg and neu """
        if sentiments:
